  ...
  return {
    prepare(query) {
      return {
        bind() { return this; },
        async all() {
          if (query.includes("cr_schema_metadata")) return { results: [{ schema_key: "control_room_history_schema", schema_version: 3, migration_name: "fixture", applied_at: "2026-07-30T00:00:00.000Z", compatibility_family: "sqlite" }] };
...
test("creates and validates a complete deterministic backup envelope", async () => {
  ...
  assert.equal(validated.databaseSchemaVersion, 3);
