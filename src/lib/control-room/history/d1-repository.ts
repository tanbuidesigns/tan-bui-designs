...
export class D1ControlRoomHistoryRepository implements ControlRoomHistoryRepository {
  ...
  async buildEvidencePacket(periodId: "28d" | "90d", generatedAt: string): Promise<ReportingEvidencePacketV1> {
    ...
    type PageEvidenceRow = { run_id: string; comparison_role: "current" | "previous"; relative_path: string; clicks: number; impressions: number; ctr: number; average_position: number };
    type DeviceEvidenceRow = { run_id: string; comparison_role: "current" | "previous"; device: "MOBILE" | "DESKTOP" | "TABLET"; clicks: number; impressions: number; ctr: number; average_position: number };
    type ChangeRow = { id: string; occurred_at: string; event_type: ChangeEventInput["eventType"]; title: string; summary: string; affected_path: string | null; lifecycle_state: ChangeEventInput["lifecycleState"]; verification_state: ChangeEventInput["verificationState"]; git_commit: string | null; worker_version_id: string | null };
    type ActionRow = { id: string; action_id: string; expected_outcome: string; observed_outcome: string; evaluation_status: ActionEvidenceInput["evaluationStatus"]; verification_state: ActionEvidenceInput["verificationState"]; baseline_run_id: string | null; followup_run_id: string | null };
    const [pageRows, deviceRows, changeRows, actionRows] = await Promise.all([
      this.db.prepare("SELECT s.run_id, s.comparison_role, p.relative_path, p.clicks, p.impressions, p.ctr, p.average_position FROM cr_search_page_rows p JOIN cr_search_snapshots s ON s.id = p.snapshot_id WHERE s.period_id = ? ORDER BY s.comparison_role, p.rank_in_returned_rows LIMIT 40").bind(periodId).all<PageEvidenceRow>(),
      this.db.prepare("SELECT s.run_id, s.comparison_role, d.device, d.clicks, d.impressions, d.ctr, d.average_position FROM cr_search_device_rows d JOIN cr_search_snapshots s ON s.id = d.snapshot_id WHERE s.period_id = ? ORDER BY s.comparison_role, d.device LIMIT 6").bind(periodId).all<DeviceEvidenceRow>(),
      this.db.prepare("SELECT id, occurred_at, event_type, title, summary, affected_path, lifecycle_state, verification_state, git_commit, worker_version_id FROM cr_change_events ORDER BY occurred_at DESC, id DESC LIMIT 20").all<ChangeRow>(),
      this.db.prepare("SELECT id, action_id, expected_outcome, observed_outcome, evaluation_status, verification_state, baseline_run_id, followup_run_id FROM cr_action_evidence ORDER BY created_at DESC, id DESC LIMIT 20").all<ActionRow>(),
    ]);
    ...
    const toMetrics = (row: { clicks: number; impressions: number; ctr: number; average_position: number } | undefined): SearchMetricSet | null => row ? { clicks: row.clicks, impressions: row.impressions, ctr: row.ctr, averagePosition: row.average_position } : null;
    const currentPages = pageRows.results.filter((row) => row.comparison_role === "current"); const previousPages = pageRows.results.filter((row) => row.comparison_role === "previous"); const pagePaths = [...new Set([...currentPages, ...previousPages].map((row) => row.relative_path))].slice(0, 20);
    const currentDevices = deviceRows.results.filter((row) => row.comparison_role === "current"); const previousDevices = deviceRows.results.filter((row) => row.comparison_role === "previous");
    return {
      ...
      pageEvidence: pagePaths.map((path) => { const current = currentPages.find((row) => row.relative_path === path); const previous = previousPages.find((row) => row.relative_path === path); return { path, current: toMetrics(current), previous: toMetrics(previous), classification: current && previous ? "present_in_both_returned_sets" as const : current ? "new_among_current_returned_top_rows" as const : "no_longer_present_in_current_returned_top_rows" as const, currentRunId: current?.run_id ?? null, previousRunId: previous?.run_id ?? null }; }),
      deviceEvidence: (["MOBILE", "DESKTOP", "TABLET"] as const).map((device) => { const current = currentDevices.find((row) => row.device === device); const previous = previousDevices.find((row) => row.device === device); return { device, current: toMetrics(current), previous: toMetrics(previous), currentRunId: current?.run_id ?? null, previousRunId: previous?.run_id ?? null }; }).filter((row) => row.current || row.previous),
      changeEvents: changeRows.results.map((row) => ({ id: row.id, occurredAt: row.occurred_at, type: row.event_type, title: row.title, summary: row.summary, affectedPath: row.affected_path, lifecycleState: row.lifecycle_state, verificationState: row.verification_state, gitCommit: row.git_commit, workerVersionId: row.worker_version_id })),
      actionEvidence: actionRows.results.map((row) => ({ id: row.id, actionId: row.action_id, expectedOutcome: row.expected_outcome, observedOutcome: row.observed_outcome, evaluationStatus: row.evaluation_status, verificationState: row.verification_state, baselineRunId: row.baseline_run_id, followupRunId: row.followup_run_id })),
      evidenceReferences: [...[currentRow, previousRow].filter((row): row is SearchRow => Boolean(row)).map((row) => ({ type: "capture-run" as const, id: row.run_id, sensitivity: "internal" as const })), ...pageSpeed.results.map((row) => ({ type: "capture-run" as const, id: row.run_id, sensitivity: "internal" as const })), ...changeRows.results.map((row) => ({ type: "change-event" as const, id: row.id, sensitivity: "internal" as const })), ...actionRows.results.map((row) => ({ type: "action-evidence" as const, id: row.id, sensitivity: "internal" as const }))],
