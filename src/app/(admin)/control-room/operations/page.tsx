...
export default function ControlRoomOperationsPage() {
  ...
  const completeTask4 = task4Readiness.filter((item) => item.complete).length;

  return (
    <ControlRoomShell activeSection="operations" eyebrow="TBD Control Room · Operations" title="Integration and security operations" description="A read-only operational view of system mode, provider readiness, configuration requirements, storage decisions and the Task 4 boundary." baselineReviewDate={snapshot.baselineReviewDate} lastUpdatedDate={snapshot.lastUpdatedDate}>
      <section aria-labelledby="system-mode-title" className="rounded-[1.35rem] border border-black/8 bg-[#111216] p-5 text-white shadow-[0_20px_60px_rgba(0,0,0,0.12)] sm:p-7">
        ...
        <dl className="mt-6 grid gap-4 text-sm sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-xl border border-white/10 bg-white/5 p-4"><dt className="text-white/42">Active mode</dt><dd className="mt-2 font-semibold">{controlRoomRuntimePolicy.activeMode}</dd></div><div className="rounded-xl border border-white/10 bg-white/5 p-4"><dt className="text-white/42">Production</dt><dd className="mt-2 font-semibold">Unavailable</dd></div><div className="rounded-xl border border-white/10 bg-white/5 p-4"><dt className="text-white/42">Local baseline</dt><dd className="mt-2 font-semibold">Active and healthy</dd></div><div className="rounded-xl border border-white/10 bg-white/5 p-4"><dt className="text-white/42">Live services</dt><dd className="mt-2 font-semibold">Disconnected</dd></div></dl>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-2"><Checklist title={`Task 4 readiness · ${completeTask4}/${task4Readiness.length}`} items={task4Readiness} /><ReadinessList title="Security readiness" items={securityReadiness} /></div>
      ...
      <section aria-labelledby="configuration-title" className="rounded-[1.35rem] border border-black/8 bg-white p-5 shadow-[0_18px_50px_rgba(0,0,0,0.04)] sm:p-7">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-500">Names only · No values created</p><h2 id="configuration-title" className="mt-2 text-2xl font-bold tracking-[-0.04em] sm:text-3xl">Future configuration manifest</h2>
