    ...
    <ControlRoomShell activeSection="operations" eyebrow="TBD Control Room · Operations" title="Integration and security operations" description="A read-only operational view of provider readiness, configuration boundaries, storage decisions and the Task 6 private Google Sign-In boundary." baselineReviewDate={snapshot.baselineReviewDate} lastUpdatedDate={snapshot.lastUpdatedDate}>
      ...
      <section aria-labelledby="task7-storage-title" className="rounded-[1.35rem] border border-black/8 bg-white p-5 shadow-[0_18px_50px_rgba(0,0,0,0.04)] sm:p-7">
        ...
        <dl className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-xl bg-[#f7f7f4] p-4"><dt className="text-xs uppercase text-gray-500">Portable backup</dt><dd className="mt-2 font-semibold">Protected local download</dd></div>
        </dl>
        <ul className="mt-5 list-disc space-y-2 pl-5 text-sm leading-relaxed text-gray-600"><li>The source-controlled D1 schema supports measurement history, the approved minimal lead register, editable action workflow state and aggregate analytics snapshots.</li><li>The portable JSON download includes schema metadata, checksums and row counts, and is validated before the browser saves it.</li><li>D1 Time Travel remains the first recovery option for recent incidents; local downloads provide an owner-controlled portable copy.</li><li>Search comparisons, homepage mobile PageSpeed and lead retention retain their bounded UTC schedules. Analysis remains a manual copy-to-ChatGPT workflow.</li></ul>
        <p className="mt-4 rounded-xl bg-[#f7f7f4] p-4 text-sm leading-relaxed text-gray-600">The download contains restricted Search evidence and personal lead records. Store it in a private encrypted folder and never commit it to the repository.</p>
        <a href="/control-room/backups/download" className="mt-5 inline-flex min-h-11 items-center rounded-lg bg-black px-5 text-sm font-semibold text-white">Download full backup</a>
      </section>

      <section className="rounded-[1.35rem] border border-black/8 bg-white p-5 shadow-[0_18px_50px_rgba(0,0,0,0.04)] sm:p-7"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-500">Tasks 8–9 · Analysis Brief and analyst</p><h2 className="mt-2 text-2xl font-bold tracking-[-0.04em] sm:text-3xl">Deterministic evidence with manual ChatGPT review</h2><dl className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-xl bg-[#f7f7f4] p-4"><dt className="text-xs uppercase text-gray-500">Source</dt><dd className="mt-2 font-semibold">ReportingEvidencePacketV1</dd></div><div className="rounded-xl bg-[#f7f7f4] p-4"><dt className="text-xs uppercase text-gray-500">Analysis</dt><dd className="mt-2 font-semibold">Copy for ChatGPT</dd></div><div className="rounded-xl bg-[#f7f7f4] p-4"><dt className="text-xs uppercase text-gray-500">Review</dt><dd className="mt-2 font-semibold">Owner-controlled</dd></div><div className="rounded-xl bg-[#f7f7f4] p-4"><dt className="text-xs uppercase text-gray-500">Dashboard billing</dt><dd className="mt-2 font-semibold">No AI provider</dd></div></dl><p className="mt-5 text-sm leading-relaxed text-gray-600">Restricted Search queries, lead details, credentials and identity remain excluded. The dashboard prepares the bounded brief but does not contact an AI service, store AI output or create actions automatically.</p></section>
